...
import { signup as localSignup, login as localLogin, currentUser, logout as localLogout } from "./auth.ts";
...
function LoginForm({ onAuthed }: { onAuthed:(u:User)=>void }) {
  ...
  const submit = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const u = await localLogin(form.name, form.password);
      saveUser(u);
      onAuthed(u);
    } catch (err: any) {
...
function SignupForm({ onAuthed }: { onAuthed:(u:User)=>void }) {
  ...
  const submit = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const u = await localSignup(form.name, form.employer, form.password);
      saveUser(u); 
      onAuthed(u);
    } catch (err: any) {
