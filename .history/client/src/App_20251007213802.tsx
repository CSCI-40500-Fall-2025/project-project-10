...
export default function App() {
  ...
  const [filters, setFilters] = useState<FiltersState>({ city: '', minPrice: null, maxPrice: null, beds: null, sort: 'priceAsc' })
...
function SignupForm({ onAuthed }: { onAuthed: (u: User) => void }) {
  ...
  return (
    <form onSubmit={submit} style={{ display: 'grid', gap: 12 }}>
      ...
      <input placeholder="Employer" value={form.employer} onChange={e => setForm(f => ({ ...f, employer: e.target.value }))} required />
      <input placeholder="Passcode" type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} required />
